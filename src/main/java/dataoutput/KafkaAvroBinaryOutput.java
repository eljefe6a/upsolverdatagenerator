...
/**
 * Outputs the UserInfo and ApacheLog objects in Avro binary format to Kafka
 * using Confluent's Avro/Kafka implementation.
 *
 */
public class KafkaAvroBinaryOutput implements DataOutput {
	/** The producer for UserInfo objects */
	KafkaProducer<String, UserInfo> userInfoProducer;

	/** The producer for ApacheLog objects */
	...
	public void writeUserInfo(UserInfo userInfo) {
		ProducerRecord<String, UserInfo> outputRecord = new ProducerRecord<>("user_info", userInfo.getUserId(),
				userInfo);
	...
	public void writeApacheLog(ApacheLog apacheLog) {
		ProducerRecord<String, ApacheLog> outputRecord = new ProducerRecord<>("apache_log", apacheLog.getUserId(),
				apacheLog);
